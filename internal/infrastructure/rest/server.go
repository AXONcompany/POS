...
func NewRouter(cfg config.Config, ingredientHandler *ingredient.IngredientHandler, productHandler *product.Handler, authHandler *auth.Handler, orderHandler *order.Handler) *gin.Engine {
	if cfg.Env == "prod" {
	...
	RegisterRouters(r, ingredientHandler, productHandler, orderHandler, jwtSecret)
