...
func NewService(productRepo ProductRepository, categoryRepo CategoryRepository, recipeRepo RecipeRepository) *Service {
	return &Service{
...
func (s *Service) CreateCategory(ctx context.Context, c product.Category) (*product.Category, error) {
	if c.Name == "" {
...
func (s *Service) GetCategory(ctx context.Context, id int64) (*product.Category, error) {
	if id <= 0 {
...
func (s *Service) GetAllCategories(ctx context.Context, page, pageSize int) ([]product.Category, error) {
	if page < 1 {
		page = 1
	}
...
func (s *Service) UpdateCategory(ctx context.Context, id int64, name string) (*product.Category, error) {
	if id <= 0 {
		return nil, product.ErrInvalidID
	}
	if name == "" {
...
func (s *Service) CreateProduct(ctx context.Context, p product.Product) (*product.Product, error) {
	if p.Name == "" {
		return nil, product.ErrNameEmpty
	}
	if p.SalesPrice < 0 {
...
func (s *Service) GetAllProducts(ctx context.Context, page, pageSize int) ([]product.Product, error) {
	if page < 1 {
		page = 1
	}
	if pageSize < 1 || pageSize > 100 {
...
func (s *Service) UpdateProduct(ctx context.Context, id int64, p product.Product) (*product.Product, error) {
	if id <= 0 {
		return nil, product.ErrInvalidID
	}
...
func (s *Service) AddIngredient(ctx context.Context, productID, ingredientID int64, quantity float64) (*product.RecipeItem, error) {
...
func (s *Service) GetProductIngredients(ctx context.Context, productID int64) ([]product.RecipeItem, error) {
	if productID <= 0 {
		return nil, product.ErrInvalidID
	}
